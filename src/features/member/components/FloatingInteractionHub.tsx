...
import { Heart, Bookmark, Share2, UserPlus, UserCheck, Presentation } from 'lucide-react';
...
export default function FloatingInteractionHub({ 
  ...
  return (
    <>
        ...
        <div className="flex flex-col items-start gap-6">
          ...
          {articleTitle && articleContent && (
            <button
              ...
              className="group/btn p-2.5 rounded-full text-zinc-400 hover:text-zinc-800 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-white/5 transition-all duration-300 relative"
            >
              <Presentation className="w-[22px] h-[22px]" />
              <div className="absolute left-full ml-3 px-2 py-1 bg-zinc-900 text-white text-[10px] font-bold rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                Tạo Slideshow
              </div>
            </button>
          )}
      ...
      <AnimatePresence>
        {showSlideshow && articleTitle && articleContent && (
          <SlideshowGenerator
            title={articleTitle}
