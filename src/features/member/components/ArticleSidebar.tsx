...
function MiniCard({ article, metric }: { article: ArticleCard; metric: React.ReactNode }) {
  return (
    <Link href={`/article/${article.slug}`} className="block group">
      <div className="flex gap-3 p-2 hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-all rounded-xl">
        <div
          className="w-14 h-14 rounded-lg shrink-0 border border-slate-100 dark:border-white/5 overflow-hidden shadow-sm bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-300 dark:text-white/20 font-bold text-lg bg-cover bg-center"
      ...
      {related.length > 0 && (
        <div>
          ...
          <div className="flex items-center gap-2 mb-4 px-2">
            ...
            <h3 className="font-display font-bold text-sm uppercase tracking-wider text-slate-900 dark:text-white">Bài viết liên quan</h3>
          </div>
          <div className="flex flex-col gap-1">
            {related.slice(0, 4).map(a => (
              <MiniCard
                ...
                article={a}
                metric={
                  <span className="text-[11px] text-slate-400 font-medium">{a.readTime} phút đọc</span>
                }
              />
            ))}
          </div>
