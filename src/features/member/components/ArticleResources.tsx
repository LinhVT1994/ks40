        ...
        <div className="space-y-2 mt-4">
          {resources.map(r => (
            <Link
              key={r.id}
              href={`/download/${r.id}`}
              className="flex items-center gap-4 p-4 rounded-2xl border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/[0.02] hover:border-primary/30 hover:bg-primary/[0.02] transition-all group"
            >
              <FileIcon mimeType={r.mimeType} />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-zinc-800 dark:text-slate-200 truncate group-hover:text-primary transition-colors">
                  {r.name}
                </p>
