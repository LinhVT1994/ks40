          ...
          <div className="max-w-xl mx-auto px-6">
            <div className="relative group">
              ...
              <div className="relative flex items-center">
                <Search className="w-5 h-5 absolute left-6 text-zinc-500 group-focus-within:text-primary transition-colors" />
                <input
                  value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                  placeholder="Tìm kiếm cuốn sách, lộ trình..."
                  className="w-full pl-14 pr-8 py-4 bg-white dark:bg-white/5 border border-zinc-300 dark:border-white/10 rounded-full outline-none focus:border-primary/30 dark:focus:border-primary/30 focus:ring-4 focus:ring-primary/5 dark:text-white shadow-lg shadow-zinc-200/20 dark:shadow-none transition-all placeholder:text-zinc-500 text-base font-medium"
                />
              </div>
            </div>
          </div>
      ...
      <section className="px-6 max-w-[1600px] mx-auto pt-24">
        <div className="flex items-center gap-3 mb-12">
          ...
          <h2 className="text-sm font-bold text-zinc-500 uppercase tracking-widest">
            Kết quả: <span className="text-zinc-800 dark:text-white">{filtered.length}</span> cuốn sách
          </h2>
        </div>
          ...
          {filtered.length === 0 && (
            <div className="col-span-full py-32 text-center">
              <div className="w-20 h-20 bg-zinc-100 dark:bg-white/5 rounded-full flex items-center justify-center mx-auto mb-6">
                <BookOpen className="w-8 h-8 text-zinc-300 dark:text-white/10" />
              </div>
              <h3 className="text-xl font-bold text-zinc-800 dark:text-white mb-2">Không tìm thấy kết quả nào</h3>
              <p className="text-zinc-500">Thử thay đổi từ khóa hoặc bộ lọc nhé!</p>
            </div>
          )}
        </div>
      </section>
