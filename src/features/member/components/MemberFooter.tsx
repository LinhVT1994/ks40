...
export default function MemberFooter() {
  ...
  if (hideOn.includes(pathname) || isMyProfile) {
    return null;
  }
      ...
      <div className="max-w-[1600px] mx-auto px-6 lg:px-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            <h4 className="font-bold text-slate-900 dark:text-white text-[11px] uppercase tracking-[0.2em]">Sản phẩm</h4>
          ...
          <div className="lg:col-span-4 space-y-6">
            <h4 className="font-bold text-slate-900 dark:text-white text-[11px] uppercase tracking-[0.2em]">Đăng ký nhận tin</h4>
            ...
            <form className="relative group max-w-sm" onSubmit={(e) => e.preventDefault()}>
              <input
                ...
                className="w-full bg-slate-100/50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary/10 transition-all text-slate-900 dark:text-white"
              />
              <button
                type="submit"
                className="absolute right-1 top-1 bottom-1 px-4 bg-primary text-white text-[11px] font-bold rounded-lg hover:bg-primary/90 transition-all active:scale-95"
              >
                Gửi ngay
              </button>
            </form>
            <div className="pt-2 flex flex-col gap-2.5 text-xs text-slate-500 dark:text-slate-400">
        ...
        <div className="mt-20 pt-8 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-8">
            <Link href="/terms" className="text-slate-400/80 hover:text-primary text-[11px] transition-colors">Điều khoản</Link>
            <Link href="/privacy" className="text-slate-400/80 hover:text-primary text-[11px] transition-colors">Bảo mật</Link>
            <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-slate-100/50 dark:bg-white/5 border border-slate-200/50 dark:border-white/5 text-[9px] font-bold text-slate-400 uppercase tracking-tighter">
...
function FooterLink({ href, label, isComingSoon = false }: { href: string; label: string; isComingSoon?: boolean }) {
  return (
    <li>
      <Link
        href={href}
        className="text-slate-500 dark:text-slate-400 hover:text-primary dark:hover:text-white text-sm transition-all hover:translate-x-1 inline-flex items-center gap-1.5 group"
      >
        <span>{label}</span>
        {isComingSoon && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-white/5 text-slate-400 font-bold uppercase tracking-widest">Sớm có</span>
        )}
      </Link>
    </li>
  );
...
function SocialLink({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <Link
      href={href}
      aria-label={label}
      className="w-10 h-10 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-400 hover:text-primary hover:border-primary/30 transition-all hover:-translate-y-1 shadow-sm"
    >
      {icon}
    </Link>
  );
