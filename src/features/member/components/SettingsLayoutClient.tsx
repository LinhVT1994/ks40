            ...
            <button
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold transition-all whitespace-nowrap min-w-fit md:min-w-0 ${
                activeTab === tab.id
                  ...
                  : 'text-zinc-600 dark:text-slate-400 hover:bg-zinc-100 dark:hover:bg-white/5 hover:text-zinc-800 dark:hover:text-white'
              } ${tab.disabled ? 'opacity-40 cursor-not-allowed hidden md:flex' : ''}`}
            >
              <tab.icon className={`w-[18px] h-[18px] transition-transform ${activeTab === tab.id ? 'text-primary scale-110' : 'text-zinc-500'}`} />
              {tab.label}
              {tab.disabled && <span className="ml-auto text-[9px] font-bold uppercase tracking-wider bg-zinc-200 dark:bg-white/10 px-1.5 py-0.5 rounded-md text-zinc-500">Coming</span>}
            </button>
          ))}
        </nav>
      </aside>
