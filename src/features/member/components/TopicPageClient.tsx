              ...
              articles.map((article, idx) => (
                <React.Fragment key={article.id}>
                  <div className="block group">
                    <div className="py-5 relative hover:bg-white dark:hover:bg-white/[0.03] hover:translate-y-[-4px] hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] flex flex-row gap-5 items-start rounded-2xl px-4 -mx-2 border border-transparent hover:border-zinc-200/50 dark:hover:border-white/5">
                      ...
                      <Link
                        ...
                        className="w-32 sm:w-40 h-24 sm:h-28 shrink-0 bg-cover bg-center relative rounded-xl overflow-hidden shadow-sm bg-zinc-100 dark:bg-white/5 border border-zinc-200 dark:border-white/5"
                      ...
                      <div className="flex-1 min-w-0 flex flex-col pt-1 transition-transform duration-500 group-hover:translate-x-1">
                        <div className="flex items-start justify-between gap-3 mb-1.5">
                          <Link href={`/article/${article.slug}`} className="flex-1">
                            <h4 className="text-base sm:text-xl font-bold text-zinc-800 dark:text-white group-hover:text-primary dark:group-hover:text-primary transition-colors font-display line-clamp-2 leading-snug" style={{ '--tw-text-opacity': '1' } as any}>
                              {article.title}
                            </h4>
                          </Link>
                        ...
                        <Link href={`/article/${article.slug}`} className="mb-4">
                          <p className="text-sm text-zinc-500 dark:text-slate-300 line-clamp-2 leading-relaxed opacity-80 group-hover:opacity-100 transition-opacity duration-500">
                            {article.summary}
                          </p>
                        </Link>

                        <div className="mt-auto flex items-center justify-between">
                          <div className="flex items-center gap-5 text-zinc-500 dark:text-slate-300 text-xs">
                            ...
                            <span className="hidden xs:flex items-center gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                          ...
                          <div className="flex items-center gap-3">
                             {article.publishedAt && (
                                <div className="hidden sm:flex items-center gap-1.5 text-[11px] font-bold text-zinc-500 dark:text-slate-400 mr-2 tracking-tight">
