...
export default function ChapterEditorClient({ book, chapter }: { book: Book, chapter: Chapter | null }) {
  ...
  return (
    <div className={
      isFullscreen
        ? "fixed inset-0 z-[100] bg-slate-50 dark:bg-slate-900 flex flex-col h-screen overflow-hidden animate-in fade-in duration-200"
        : "flex-1 flex flex-col w-full h-full overflow-hidden bg-slate-50 dark:bg-slate-900 relative"
    }>
      
      {/* HEADER BAR */}
      <div className="flex items-center justify-between px-6 py-4 shrink-0 border-b border-slate-200 dark:border-white/5 bg-transparent z-10">
        <div className="flex items-center gap-4 flex-1 min-w-0">
          <Link 
            href={`/admin/books/${book.id}/chapters`}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white dark:bg-white/5 text-slate-500 hover:text-primary transition-all shrink-0 tooltip-trigger shadow-sm group"
          ...
          <div className="flex items-center gap-2 text-sm text-slate-400 flex-1 min-w-0 pr-4">
            <span className="hidden sm:inline shrink-0 font-bold bg-slate-200 dark:bg-white/10 px-3 py-1.5 rounded-lg text-slate-600 dark:text-slate-300 shadow-inner">
              {book.title}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button 
            ...
            className="p-3 text-slate-400 hover:text-primary hover:bg-slate-100 dark:hover:bg-white/5 rounded-xl transition-all font-bold group bg-white dark:bg-transparent shadow-sm border border-slate-200 dark:border-transparent"
          ...
          <button onClick={handleSave} disabled={formPending} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-slate-900 dark:bg-primary text-white text-sm font-bold hover:opacity-90 transition-all disabled:opacity-50 shadow-lg shadow-slate-900/10 dark:shadow-primary/20 hover:-translate-y-0.5 active:translate-y-0">
      ...
      <div className="shrink-0 bg-transparent">
        ...
        {/* Inline Title Input */}
        <div className="px-6 py-2 bg-transparent border-b border-slate-100 dark:border-white/5">
          <input 
            value={form.title}
            onChange={e => setF('title', e.target.value)}
            placeholder="Nhập tiêu đề chương..."
            className="w-full text-lg font-bold text-slate-800 dark:text-white bg-transparent border-none outline-none placeholder:text-slate-300 dark:placeholder:text-slate-600 transition-all px-0"
          />
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {(viewMode === 'editor' || viewMode === 'split') && (
          <div className={`flex flex-col ${viewMode === 'split' ? 'w-1/2 border-r border-slate-200 dark:border-white/5' : 'flex-1'} bg-transparent`}>
            <div className="flex items-center px-4 py-2 bg-transparent border-b border-slate-100 dark:border-white/5 shrink-0 z-10">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                Markdown
              </span>
              <span className="ml-auto text-[10px] text-slate-400">{form.content?.length || 0} ký tự</span>
            </div>
            <textarea
              ref={textareaRef}
              value={form.content}
              onChange={e => setF('content', e.target.value)}
              className="flex-1 w-full resize-none p-6 font-mono text-sm leading-relaxed outline-none bg-transparent text-slate-800 dark:text-slate-200 placeholder:text-slate-300 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-white/10"
              spellCheck={false}
