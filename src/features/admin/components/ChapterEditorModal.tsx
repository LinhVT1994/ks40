    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="w-full max-w-6xl h-full max-h-[90vh] bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden border border-white/20">
        ...
        <div className="px-8 py-5 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-white/[0.02]">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Soạn thảo nội dung Markdown</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
             <button 
               onClick={onClose}
               className="p-2 rounded-xl text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5 transition-all"
             >
               <X className="w-5 h-5" />
             </button>
          </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          {/* Controls */}
          <div className="px-8 py-4 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-white/5 flex flex-wrap items-center justify-between gap-4">
             <div className="flex-1 max-w-md relative group">
                <Type className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary transition-colors" />
                <input 
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  placeholder="Tiêu đề chương..."
                  className="w-full pl-10 pr-4 py-2.5 text-sm bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-2xl outline-none focus:ring-2 focus:ring-primary/20 dark:text-white font-bold"
                />
             </div>
             ...
             <div className="flex items-center gap-1 bg-slate-100 dark:bg-white/5 p-1 rounded-2xl">
                <button 
                  ...
                  className={`flex items-center gap-2 px-4 py-1.5 rounded-xl text-xs font-bold transition-all ${view === 'edit' ? 'bg-white dark:bg-slate-800 text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  <Edit3 className="w-3.5 h-3.5" /> Soạn thảo
                </button>
                <button 
                  onClick={() => setView('preview')}
                  className={`flex items-center gap-2 px-4 py-1.5 rounded-xl text-xs font-bold transition-all ${view === 'preview' ? 'bg-white dark:bg-slate-800 text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
             ...
             <button 
               ...
               className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-xs font-bold transition-all border ${
                 isFree 
                   ...
                   : 'bg-slate-50 border-slate-100 text-slate-400 dark:bg-white/5 dark:border-white/5 dark:text-slate-500 hover:border-slate-300'
               }`}
             >
          ...
          <div className="flex-1 relative overflow-hidden">
            {view === 'edit' ? (
              <textarea 
                ...
                className="w-full h-full p-8 text-sm bg-transparent outline-none dark:text-slate-300 font-mono leading-relaxed resize-none placeholder:text-slate-300 dark:placeholder:text-slate-700"
              />
            ) : (
              <div className="w-full h-full p-8 overflow-y-auto prose prose-slate dark:prose-invert max-w-none">
                {content ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-slate-300 dark:text-white/10 italic">
        ...
        {/* Modal Footer */}
        <div className="px-8 py-5 border-t border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-white/[0.02]">
