    ...
    <div className="bg-white dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-3xl overflow-hidden">
      <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 dark:border-white/5">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-display font-bold text-slate-900 dark:text-white">Top bài viết</h3>
        </div>
      ...
      <div className="divide-y divide-slate-100 dark:divide-white/5">
        {articles.map((article, idx) => (
          <div key={article.id} className="flex items-center gap-4 px-6 py-3.5 hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
            <span className="text-lg font-bold text-slate-200 dark:text-white/10 w-5 shrink-0 text-center">{idx + 1}</span>
            <div className="flex-1 min-w-0">
              <Link
                href={`/article/${article.slug}`}
                target="_blank"
                className="text-sm font-semibold text-slate-900 dark:text-white group-hover:text-primary transition-colors block truncate"
              >
                {article.title}
              </Link>
              <div className="flex items-center gap-2 mt-0.5">
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full uppercase tracking-wider ${CATEGORY_COLORS[article.category] ?? 'bg-slate-100 text-slate-500'}`}>
