        ...
        {authors.map((author, idx) => {
          ...
          return (
            <div key={author.id} className="flex items-center gap-3 px-6 py-3.5 hover:bg-zinc-50 dark:hover:bg-white/[0.02] transition-colors group">
              ...
              <Link href={`/@${author.username ?? author.id}`} className="shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <Link href={`/@${author.username ?? author.id}`} className="text-sm font-semibold text-zinc-800 dark:text-white group-hover:text-primary transition-colors block truncate">
                  {author.name}
                </Link>
                {/* Progress bar */}
