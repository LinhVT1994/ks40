...
export default function TopAuthors({ authors }: { authors: Author[] }) {
  ...
  return (
    <div className="bg-white dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-3xl overflow-hidden">
      <div className="flex items-center gap-2 px-6 py-5 border-b border-slate-100 dark:border-white/5">
        <Users2 className="w-4 h-4 text-emerald-500" />
        <h3 className="font-display font-bold text-slate-900 dark:text-white">Top tác giả</h3>
      </div>
      <div className="divide-y divide-slate-100 dark:divide-white/5">
        {authors.map((author, idx) => {
          ...
          return (
            <div key={author.id} className="flex items-center gap-3 px-6 py-3.5 hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
              <span className="text-sm font-bold text-slate-200 dark:text-white/10 w-4 shrink-0 text-center">{idx + 1}</span>
              ...
              <div className="flex-1 min-w-0">
                <Link href={`/profile/${author.id}`} className="text-sm font-semibold text-slate-900 dark:text-white group-hover:text-primary transition-colors block truncate">
                  {author.name}
                </Link>
                {/* Progress bar */}
                <div className="mt-1 h-1 bg-slate-100 dark:bg-white/10 rounded-full overflow-hidden w-full">
                  <div
