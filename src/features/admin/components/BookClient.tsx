    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="relative flex-1 max-w-md w-full">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 text-sm bg-white dark:bg-white/5 border border-zinc-300 dark:border-white/10 rounded-2xl outline-none focus:ring-2 focus:ring-primary/20 dark:text-white placeholder:text-zinc-500 shadow-sm"
          />
        </div>
        <Link
      ...
      <div className="grid grid-cols-1 gap-4">
        {filtered.map(book => (
          <div
            key={book.id}
            className="flex flex-col md:flex-row gap-6 p-5 bg-white dark:bg-white/[0.02] border border-zinc-200 dark:border-white/5 rounded-3xl hover:border-primary/30 dark:hover:border-primary/20 transition-all group"
          >
            {/* Cover */}
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  <h3 className="text-lg font-bold text-zinc-800 dark:text-white truncate font-display group-hover:text-primary transition-colors">
                    {book.title}
                  </h3>
                  {book.published ? (
                    ...
                    <span className="text-[10px] font-bold text-zinc-500 bg-zinc-100 dark:bg-white/5 px-2 py-0.5 rounded-full uppercase tracking-wider">Nháp</span>
                  )}
                </div>
                <p className="text-sm text-zinc-500 dark:text-slate-400 line-clamp-2 leading-relaxed max-w-2xl">
              ...
              <div className="flex flex-wrap items-center gap-y-3 gap-x-6 mt-4 md:mt-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-zinc-600 dark:text-slate-300">{book._count.chapters} Chapters</span>
                </div>
                <span className="text-xs text-zinc-500">Tạo bởi {book.author.name}</span>
                <span className="text-xs text-zinc-500">{new Intl.DateTimeFormat('vi-VN').format(new Date(book.createdAt))}</span>
              </div>
            </div>

            {/* Actions */}
            <div className="flex items-center md:flex-col justify-end gap-2 md:pl-6 md:border-l md:border-zinc-200 md:dark:border-white/5">
              <Link
                href={`/admin/books/${book.id}/edit`}
                className="flex items-center gap-2 px-4 py-2 text-xs font-bold text-zinc-600 dark:text-slate-300 hover:text-primary hover:bg-primary/5 rounded-xl transition-all"
              >
              ...
              <button
                ...
                className="p-2 rounded-xl text-zinc-500 hover:text-rose-500 hover:bg-rose-500/5 transition-all disabled:opacity-40"
                title="Xoá book"
              >
        ...
        {filtered.length === 0 && (
          <div className="py-24 text-center">
            <div className="inline-flex p-4 bg-zinc-100 dark:bg-white/5 rounded-full mb-4">
              <Book className="w-8 h-8 text-zinc-300 dark:text-white/20" />
            </div>
            <h4 className="text-zinc-800 dark:text-white font-bold">Chưa có Book nào</h4>
            <p className="text-sm text-zinc-500 mt-1">Bắt đầu bằng cách tạo một cuốn sách mới!</p>
          </div>
        )}
      </div>
    </div>
