  ...
  return (
    <div className="bg-white dark:bg-white/[0.02] border border-zinc-200 dark:border-white/5 rounded-3xl overflow-hidden">
      <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-200 dark:border-white/5">
        <h3 className="font-display font-bold text-zinc-800 dark:text-white">Bài viết gần đây</h3>
      ...
      <div className="divide-y divide-zinc-200 dark:divide-white/5">
        {articles.map(doc => (
          <div key={doc.id} className="flex items-center gap-4 px-6 py-4 hover:bg-zinc-50 dark:hover:bg-white/[0.02] transition-colors group">
            <div className="flex-1 min-w-0">
              <Link href={`/article/${doc.slug}`} target="_blank" className="text-sm font-bold text-zinc-800 dark:text-white truncate mb-1 group-hover:text-primary transition-colors block">
                {doc.title}
              </Link>
              <div className="flex items-center gap-3">
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${CATEGORY_COLORS[doc.category] ?? 'bg-zinc-100'}`}>
            ...
            <div className="text-right shrink-0">
              <p className="text-sm font-bold text-zinc-800 dark:text-white">{doc.viewCount.toLocaleString()}</p>
              <p className="text-[11px] text-zinc-500">lượt xem</p>
            </div>
        ...
        {articles.length === 0 && (
          <div className="py-10 text-center text-sm text-zinc-500">Chưa có bài viết nào.</div>
        )}
      </div>
    </div>
  );
