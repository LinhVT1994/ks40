...
export default function CommentDetailModal({ comment, onClose, onToggleStatus, onMarkSpam, onDelete }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-slate-900 rounded-2xl border border-zinc-300 dark:border-white/10 shadow-2xl w-full max-w-lg animate-in fade-in zoom-in-95 duration-200"
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200 dark:border-white/5">
          <div className="flex items-center gap-3">
            <div
              className="w-10 h-10 rounded-full bg-cover bg-center border border-zinc-300 dark:border-white/10 shrink-0"
            ...
            />
            <div>
              <p className="text-sm font-bold text-zinc-800 dark:text-white">{comment.author.name}</p>
              <p className="text-xs text-zinc-500">{comment.author.email}</p>
            </div>
        ...
        <div className="px-6 py-5 space-y-4">
          {/* Content */}
          <div>
            <p className="text-[11px] font-bold uppercase tracking-widest text-zinc-500 mb-2">Nội dung bình luận</p>
          ...
          {/* Article context */}
          <div>
            <p className="text-[11px] font-bold uppercase tracking-widest text-zinc-500 mb-2">Bài viết liên quan</p>
            <div className="bg-zinc-50 dark:bg-white/5 rounded-xl p-4 border border-zinc-200 dark:border-white/5">
              ...
              <p className="text-xs text-zinc-500 mt-1">/{comment.article.slug}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 px-6 py-4 border-t border-zinc-200 dark:border-white/5 bg-zinc-50/50 dark:bg-white/[0.01] rounded-b-2xl">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl border border-zinc-300 dark:border-white/10 text-zinc-600 dark:text-slate-400 hover:bg-white dark:hover:bg-white/5 transition-colors"
          >
