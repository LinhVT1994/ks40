...
const audiences: Audience[] = [
  ...
  { id: 'private', label: 'Riêng tư', desc: 'Chỉ mình bạn và Admin xem được', icon: Lock, color: 'text-slate-500' },
];
    ...
    <div className="flex-1 overflow-y-auto bg-transparent p-6 md:p-12">
      <div className="max-w-5xl mx-auto flex flex-col min-h-full">
        ...
        <div className="mb-10 border-b border-slate-100 dark:border-white/5 pb-6">
          <h2 className="text-3xl font-bold text-slate-900 dark:text-white font-display mb-2">Chia sẻ & Xuất bản</h2>
          <p className="text-slate-500">Cấu hình người xem và thời gian phát hành bài viết của bạn.</p>
        </div>
        ...
        <div className="space-y-12 flex-1">
          ...
          <div className="space-y-4">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2 mb-6">
                  ...
                  <button key={a.id} onClick={() => setSelectedAudience(a.id)}
                    className={`flex flex-col items-start gap-4 p-5 rounded-2xl border-2 text-left transition-all relative overflow-hidden ${
                      isSelected
                        ...
                        : 'border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 hover:border-primary/30'
                    }`}>
                    ...
                    <div>
                      <p className={`font-bold ${isSelected ? 'text-primary' : 'text-slate-900 dark:text-white'}`}>{a.label}</p>
          ...
          <div className="space-y-4">
                ...
                <button key={opt.id} onClick={() => setScheduleType(opt.id)}
                  className={`flex-1 py-4 px-6 rounded-2xl text-sm font-bold border-2 transition-all text-center ${
                    scheduleType === opt.id
                      ...
                      : 'border-slate-200 dark:border-white/10 text-slate-500 bg-slate-50 dark:bg-white/5 hover:border-primary/30'
                  }`}>
                  {opt.label}
                </button>
              ))}
            ...
            {scheduleType === 'later' && (
              <div className="flex items-center gap-4 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-5 py-4 transition-all mt-4">
                <Calendar className="w-5 h-5 text-slate-400 shrink-0" />
                <input
                  type="datetime-local"
                  value={scheduleDate}
                  onChange={(e) => setScheduleDate(e.target.value)}
                  className="flex-1 bg-transparent text-sm outline-none dark:text-white text-slate-700 font-medium"
                />
              </div>
            )}
          </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1">
                <select
                  ...
                  className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm text-slate-700 dark:text-white outline-none focus:border-primary transition-colors"
                >
              ...
              {selectedSeriesId && (
                <div className="flex items-center gap-3 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 sm:w-48">
                  ...
                  <input
                    ...
                    className="w-full bg-transparent text-sm outline-none dark:text-white text-slate-700 font-medium"
                  />
                </div>
              )}
            </div>
            ...
            {nextArticleId ? (
              <div className="flex items-center gap-3 p-4 rounded-xl bg-primary/5 border border-primary/20">
                ...
                <button
                  ...
                  className="shrink-0 px-3 py-1.5 rounded-lg text-xs font-bold text-slate-500 hover:text-rose-500 border border-slate-200 dark:border-white/10 hover:border-rose-300 transition-colors"
                >
                  Xoá
                </button>
              </div>
              ...
              <div className="space-y-2">
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm outline-none focus:border-primary transition-colors dark:text-white"
                />
                {articleSearch && (
                  <div className="max-h-52 overflow-y-auto rounded-xl border border-slate-200 dark:border-white/10 divide-y divide-slate-100 dark:divide-white/5 bg-white dark:bg-slate-900 shadow-lg">
                    {articlePickerList
                      ...
                      .map(a => (
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-4 py-3 text-left text-sm hover:bg-slate-50 dark:hover:bg-white/5 transition-colors"
                        >
                          {a.thumbnail && (
                            <img src={a.thumbnail} alt="" className="w-10 h-7 rounded-md object-cover shrink-0" />
                          )}
                          <span className="truncate text-slate-700 dark:text-slate-200 font-medium">{a.title}</span>
                        </button>
                      ))}
                    {articlePickerList.filter(a => a.title.toLowerCase().includes(articleSearch.toLowerCase())).length === 0 && (
                      <p className="px-4 py-3 text-sm text-slate-400">Không tìm thấy bài viết nào</p>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="space-y-4">
            ...
            <button onClick={() => setSendNotif(!sendNotif)}
              className={`w-full flex items-center justify-between gap-4 p-5 rounded-2xl border-2 text-left transition-all ${
                sendNotif ? 'border-primary bg-primary/5 dark:bg-primary/10' : 'border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5'
              }`}>
              <div className="flex items-center gap-5">
                <div className={`p-3.5 rounded-xl shadow-sm ${sendNotif ? 'bg-white dark:bg-white/10 text-primary' : 'bg-white dark:bg-white/10 text-slate-400'}`}>
                  <Bell className="w-6 h-6" />
                </div>
                <div>
                  <p className={`font-bold ${sendNotif ? 'text-primary' : 'text-slate-700 dark:text-slate-300'}`}>
                    Gửi thông báo đẩy cho người theo dõi
                  </p>
                  <p className="text-sm text-slate-500 mt-1">Người dùng sẽ nhận được email ngay khi bài viết của bạn được công khai.</p>
                </div>
              </div>
              <div className={`w-12 h-6 rounded-full transition-colors relative shrink-0 ${sendNotif ? 'bg-primary' : 'bg-slate-300 dark:bg-slate-700'}`}>
        ...
        {/* Footer Navigation */}
        <div className="pt-6 mt-12 border-t border-slate-100 dark:border-white/5 flex items-center justify-between shrink-0">
          <button onClick={onBack} className="flex items-center gap-2 px-6 py-3 rounded-xl border border-slate-200 dark:border-white/10 text-sm font-bold text-slate-500 hover:text-slate-900 dark:hover:text-white hover:border-slate-300 dark:hover:border-white/20 transition-all">
            <ArrowLeft className="w-4 h-4" /> Quay lại
          </button>
          <div className="flex items-center gap-4">
            <button className="flex items-center gap-2 px-6 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 text-sm font-bold text-slate-700 dark:text-white hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm">
              <Eye className="w-4 h-4" /> Xem trước
            </button>
            <button onClick={() => onPublish(selectedAudience, scheduleType, scheduleDate, resources, selectedSeriesId, seriesOrder ? parseInt(seriesOrder, 10) : null, nextArticleId)} className="flex items-center gap-2 px-8 py-3 rounded-xl bg-slate-900 dark:bg-primary text-white text-sm font-bold hover:opacity-90 transition-all shadow-lg shadow-black/10 dark:shadow-primary/20">
