...
const AUDIENCE = [
  ...
  { value: 'PRIVATE', label: 'Riêng tư',  icon: Lock,  cls: 'text-slate-400',   badge: 'bg-slate-100 text-slate-500 dark:bg-white/5 dark:text-slate-400' },
];
const EXPIRES = [
    ...
    <>
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <Link href="/admin/shares/new"
          className="flex items-center gap-2 px-4 py-2.5 text-sm font-bold bg-slate-900 dark:bg-primary text-white rounded-2xl hover:opacity-90 transition-all shrink-0">
          <Plus className="w-4 h-4" /> Tạo gói mới
        </Link>
      </div>

      {/* ── Table ── */}
      <div className="bg-white dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-lg shadow-sm flex flex-col min-w-[700px]">
          {/* Header */}
          <div className="grid grid-cols-12 px-6 py-3 border-b border-slate-100 dark:border-white/5 text-[11px] font-bold uppercase tracking-widest text-slate-400 bg-slate-50/90 dark:bg-slate-900/90 backdrop-blur-md rounded-t-lg items-center sticky top-[65px] md:top-[73px] z-20">
          ...
          <div className="divide-y divide-slate-100 dark:divide-white/5">
            {filtered.length === 0 ? (
              ...
              filtered.map(pkg => {
                ...
                return (
                  <div key={pkg.id} className="grid grid-cols-12 px-6 py-4 hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                    {/* Title + actions */}
                    <div className="col-span-5 pr-4 min-w-0 h-full flex flex-col justify-center">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <p className="text-sm font-bold text-slate-900 dark:text-white truncate flex-1 group-hover:text-primary transition-colors">
                          {pkg.title}
                        </p>
                        <div className="flex items-center gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                          <CopyBtn path={sharePath} />
                          <a href={sharePath} target="_blank" rel="noopener noreferrer"
                            className="p-1.5 rounded-lg text-slate-400 hover:text-primary hover:bg-slate-100 dark:hover:bg-white/5 transition-colors">
                      ...
                      {pkg.description && (
                        <p className="text-[11px] text-slate-400 mt-0.5 truncate">{pkg.description}</p>
                      )}
                      <p className="text-[11px] text-slate-400 mt-0.5">{fmtDate(pkg.createdAt)} · {pkg.uploadedBy.name}</p>
                    </div>

                    {/* Files + downloads */}
                    <div className="col-span-2 h-full flex flex-col items-center justify-center gap-0.5">
                      <span className="text-sm font-bold text-slate-700 dark:text-slate-300">{pkg.files.length} file · {fmt(totalSize)}</span>
                      ...
                      {expired
                        ...
                        : pkg.expiresAt
                          ? <><span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400">Khả dụng</span>
                            <span className="text-[10px] text-slate-400">{fmtDate(pkg.expiresAt)}</span></>
                          : <><span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400">Khả dụng</span>
                            <span className="text-[10px] text-slate-400">Không giới hạn</span></>
                      }
                    </div>
                  </div>
                );
