...
import {
  Bell, FileText, MessageSquare, Heart, Zap,
          ...
          {TABS.map(({ key, label, icon: Icon }) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 text-sm font-semibold relative transition-colors ${
                tab === key ? 'text-primary' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'
              }`}
            >
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center flex-wrap">
            ...
            <div className="relative flex-1 max-w-sm">
              ...
              {q && <button type="button" onClick={() => { setQ(''); navigate({ q: '', page: '1' }); }}
                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600"><X className="w-3.5 h-3.5" /></button>}
            </div>

            <div className="flex items-center gap-1 flex-wrap">
              <button onClick={() => navigate({ type: 'all' })}
                className={`px-3 py-2 rounded-full text-xs font-semibold border transition-all ${currentType === 'all' ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 border-transparent' : 'bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-500 hover:border-slate-300'}`}>
                Tất cả
              </button>
              {NOTIF_TYPES.map(t => {
                ...
                return (
                  <button key={t.key} onClick={() => navigate({ type: t.key })}
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-full text-xs font-semibold border transition-all ${isActive ? `${cfg.bg} ${cfg.color} border-current` : 'bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-500 hover:border-slate-300'}`}>
            ...
            <div className="flex items-center gap-1 bg-slate-100 dark:bg-white/5 p-0.5 rounded-2xl sm:ml-auto">
              {READ_FILTERS.map(f => (
                <button key={f.key} onClick={() => navigate({ read: f.key })}
                  className={`px-3 py-2 rounded-xl text-xs font-semibold transition-all ${currentRead === f.key ? 'bg-white dark:bg-white/10 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                  {f.label}
                </button>
              ))}
            </div>
            ...
            {stats.total - stats.unread > 0 && (
              <button onClick={handleDeleteRead} disabled={isPending}
                className="flex items-center gap-1.5 px-3 py-2.5 rounded-2xl text-xs font-semibold text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 border border-transparent hover:border-rose-200 dark:hover:border-rose-500/20 transition-all">
          ...
          <div className={`transition-opacity ${isPending ? 'opacity-50' : 'opacity-100'}`}>
            {notifications.length === 0 ? (
              ...
              <div className="bg-white dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-2xl overflow-hidden">
                <div className="grid grid-cols-[1fr_180px_100px_90px_44px] gap-4 px-5 py-3 border-b border-slate-100 dark:border-white/5 text-xs font-bold text-slate-400 uppercase tracking-wider">
                ...
                <div className="divide-y divide-slate-50 dark:divide-white/5">
                  {notifications.map(n => {
                    ...
                    return (
                      <div key={n.id} className={`grid grid-cols-[1fr_180px_100px_90px_44px] gap-4 px-5 py-3.5 items-center hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors ${!n.read ? 'bg-primary/[0.02]' : ''}`}>
                        <div className="flex items-start gap-3 min-w-0">
                          <div className={`p-1.5 rounded-lg shrink-0 ${cfg.bg}`}>
                            <Icon className={`w-3.5 h-3.5 ${cfg.color}`} />
                          </div>
                          <div className="min-w-0">
                            <p className={`text-sm truncate ${!n.read ? 'font-semibold text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-300'}`}>
                              {n.title}
                        ...
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-xs font-medium text-slate-700 dark:text-slate-300 truncate">{n.user.name}</p>
                            <p className="text-[10px] text-slate-400 truncate">{n.user.email}</p>
                          </div>
                        </div>
