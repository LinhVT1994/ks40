      ...
      <section className="px-5 md:px-12 py-16 md:py-24 max-w-[1600px] mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-start relative">
          ...
          <div className="lg:col-span-8 space-y-12 md:space-y-20">
            <div>
               <h2 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white font-display mb-6 tracking-tight">
                 Lộ trình học tập
               </h2>
            ...
            <div className="space-y-0 border-t border-slate-100 dark:border-white/5">
               {book.chapters.map((chapter) => (
                 <Link
                   key={chapter.id}
                   href={`/books/${book.slug}/${chapter.slug}`}
                   className="flex items-center gap-5 md:gap-8 py-7 md:py-8 border-b border-slate-100 dark:border-white/5 transition-all group px-4 -mx-4 rounded-2xl hover:bg-slate-50 dark:hover:bg-white/[0.02]"
                 >
                   <div className="text-2xl font-black text-slate-200 dark:text-slate-800 w-10 shrink-0 group-hover:text-primary/40 transition-colors tabular-nums">
                      {chapter.order < 10 ? `0${chapter.order}` : chapter.order}
                   </div>

                   <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-1.5">
                         <h3 className="text-lg md:text-xl font-bold text-slate-800 dark:text-white group-hover:text-primary transition-colors truncate tracking-tight">
                           {chapter.title}
                         </h3>
                         {chapter.isFree && (
                   ...
                   <div className="flex items-center gap-6 shrink-0">
                      {!chapter.isFree && <Lock className="w-4 h-4 text-slate-300 dark:text-slate-700" />}
                      <div className="w-10 h-10 rounded-full bg-slate-50 dark:bg-white/5 flex items-center justify-center text-slate-300 group-hover:bg-primary group-hover:text-white transition-all transform group-hover:translate-x-1">
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-32 space-y-10">
            {/* Author Card */}
            <div className="p-10 bg-white dark:bg-white/[0.02] border border-slate-200/60 dark:border-white/5 rounded-[3rem] shadow-sm flex flex-col items-center text-center group hover:border-primary/20 transition-all duration-700 backdrop-blur-xl">
               ...
               <h3 className="text-2xl font-black text-slate-900 dark:text-white font-display mb-1.5 tracking-tight">{book.author.name}</h3>

               {book.author.bio && (
                 <p className="text-base text-slate-500 dark:text-slate-400 leading-relaxed mb-10 px-4 font-medium italic">
                   &ldquo;{book.author.bio}&rdquo;
                 </p>
               )}

               <button className="w-full py-5 bg-slate-950 dark:bg-white text-white dark:text-slate-950 rounded-[1.5rem] text-xs font-black uppercase tracking-[0.25em] hover:bg-primary dark:hover:bg-primary dark:hover:text-white transition-all shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)] dark:shadow-none flex items-center justify-center gap-3 active:scale-95">
            ...
            <div className="grid grid-cols-2 gap-5">
               <div className="p-8 bg-white dark:bg-white/[0.02] border border-slate-200/60 dark:border-white/5 rounded-[2.5rem] text-center hover:border-primary/20 transition-all group shadow-sm active:scale-95">
                  <div className="text-3xl font-black text-slate-900 dark:text-white mb-1 group-hover:text-primary transition-colors tracking-tighter">2.4k</div>
                  <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center justify-center gap-2">
                    <Users className="w-4 h-4" /> Students
                  </div>
               </div>
               <div className="p-8 bg-white dark:bg-white/[0.02] border border-slate-200/60 dark:border-white/5 rounded-[2.5rem] text-center hover:border-primary/20 transition-all group shadow-sm active:scale-95">
                  <div className="text-3xl font-black text-slate-900 dark:text-white mb-1 group-hover:text-primary transition-colors tracking-tighter">4.9/5</div>
                  <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center justify-center gap-2">
            ...
            <div className="p-10 bg-primary/5 dark:bg-primary/10 border border-primary/10 dark:border-white/5 rounded-[3rem] relative overflow-hidden group shadow-sm hover:shadow-md transition-all">
               ...
               <div className="relative z-10">
                 <div className="w-14 h-14 bg-white dark:bg-white/5 rounded-2xl flex items-center justify-center mb-8 border border-slate-100 dark:border-white/5 shadow-sm">
                   <Info className="w-7 h-7 text-primary" />
                 </div>
                 <h4 className="text-2xl font-black text-slate-900 dark:text-white mb-4 font-display tracking-tight">Tư vấn hỗ trợ</h4>
                 <p className="text-base text-slate-500 dark:text-slate-400 leading-relaxed mb-10 font-medium">
                   Nếu bạn cần hỗ trợ về lộ trình học tập, đừng ngần ngại chat với đội ngũ chuyên gia của chúng tôi.
                 </p>
                 <button className="w-full py-5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[1.5rem] text-xs font-black uppercase tracking-[0.3em] shadow-xl shadow-slate-900/10 dark:shadow-none hover:scale-[1.02] active:scale-95 transition-all">
                   Chat với Support
                 </button>
               </div>
            </div>
