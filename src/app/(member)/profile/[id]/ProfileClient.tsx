          ...
          <div className="hidden lg:block lg:sticky lg:top-1/2 lg:-translate-y-1/2 lg:ml-[50px] flex-shrink-0 w-[48px] h-fit order-3">
            <div className="flex flex-col items-center gap-4 animate-in fade-in slide-in-from-right-10 duration-1000 delay-300">
              {SOCIAL_LINKS.map(({ key, icon: Icon, label }) => {
                ...
                if (!url) return null;
                return (
                  <a key={key} href={url} target="_blank" rel="noopener noreferrer" title={label} className="group p-3 rounded-full text-zinc-400 bg-surface/60 backdrop-blur-md border border-zinc-100 dark:border-white/5 transition-all hover:-translate-x-1 hover:text-primary hover:border-primary/20 shadow-sm">
