      ...
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-start">
        ...
        <div className="flex flex-col gap-5">
          <div>
            ...
            <h1 className="text-3xl font-bold text-zinc-800 dark:text-white leading-tight">{pkg.title}</h1>
            {pkg.description && (
              <p className="text-sm text-zinc-500 dark:text-slate-400 leading-relaxed mt-3">{pkg.description}</p>
            )}
          </div>
          ...
          {!ADSENSE_CLIENT && !isLoggedIn && (
            <div className="h-28 rounded-2xl border-2 border-dashed border-zinc-300 dark:border-white/10 flex items-center justify-center text-zinc-500 text-xs mt-2">
        ...
        <div className="flex flex-col gap-4">
          {/* File list */}
          <div className="bg-white/60 dark:bg-white/[0.03] border border-zinc-200 dark:border-white/10 rounded-2xl overflow-hidden">
            <div className="divide-y divide-zinc-200 dark:divide-white/5">
              {pkg.files.map(f => (
                <div key={f.id} className="flex items-center gap-3 px-5 py-3.5 hover:bg-zinc-50 dark:hover:bg-white/[0.03] transition-colors">
                  <div className="w-9 h-9 rounded-xl bg-zinc-50 dark:bg-white/5 flex items-center justify-center shrink-0">
                    <FileIcon mimeType={f.mimeType} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-zinc-800 dark:text-slate-200 truncate">{f.name}</p>
                  ...
                  {ready ? (
                    ...
                    <Lock className="w-3.5 h-3.5 text-zinc-300 dark:text-white/20 shrink-0" />
                  )}
                </div>
              ))}
            </div>
          ...
          {ready ? (
            ...
            <div className="bg-white/60 dark:bg-white/[0.03] border border-zinc-200 dark:border-white/10 rounded-2xl px-6 py-5 flex items-center gap-5">
              <div className="relative w-14 h-14 shrink-0">
                ...
                <span className="absolute inset-0 flex items-center justify-center text-lg font-bold text-zinc-800 dark:text-white">{count}</span>
              </div>
              <div>
                <p className="text-sm font-semibold text-zinc-700 dark:text-slate-300 flex items-center gap-1.5">
                  <Clock className="w-3.5 h-3.5 text-zinc-500" /> Vui lòng chờ {count} giây…
                </p>
