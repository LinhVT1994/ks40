...
export default function GlobalNotesSidebar() {
  ...
  // Pre-computed normalized haystack per annotation. Rebuilt only when annotations change.
  ...
  // Search-only filtered set (shared across tabs for counts)
  ...
  // Counts per tab (respects search)
  ...
  // Final filtered list (type + search)
  ...
  // Reset visible count when query/filter/sidebar state changes
  useEffect(() => {
      ...
      {isSidebarOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 h-full bg-white dark:bg-slate-900 border-l border-zinc-200 dark:border-white/10 z-[210] shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Resize handle */}
            <div
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      <div className="relative group flex-1">
                        ...
                        <input
                          ...
                          className="w-full bg-zinc-100 dark:bg-black/20 border border-transparent focus:border-primary/50 dark:focus:border-primary/50 text-[13px] text-zinc-800 dark:text-slate-200 rounded-xl pl-10 pr-16 py-2.5 outline-none transition-all placeholder:text-zinc-500"
                        />
                        {searchQuery ? (
                          <button
                      ...
                      <button 
                        ...
                        className="p-2.5 shrink-0 bg-primary/10 hover:bg-primary/20 dark:bg-primary/20 dark:hover:bg-primary/30 text-primary border border-primary/20 dark:border-primary/30 rounded-xl transition-all hover:scale-105 active:scale-95 flex items-center justify-center"
                      >
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-3">
                    {loading ? (
                      ...
                      debouncedQuery ? (
                        <div className="h-full flex flex-col items-center justify-center text-center px-6 opacity-50 gap-2">
                          <SearchX className="w-10 h-10" />
                          <p className="text-sm font-bold uppercase tracking-widest">Không tìm thấy</p>
                      ...
                      <>
                        ...
                        {hasMore && (
                          <button
                            ...
                            className="group/more w-full mt-2 py-3 rounded-2xl border border-dashed border-zinc-200 dark:border-white/10 hover:border-primary/40 dark:hover:border-primary/40 hover:bg-primary/[0.03] transition-all flex items-center justify-center gap-2 text-[11px] font-black uppercase tracking-[0.16em] text-zinc-500 dark:text-zinc-400 hover:text-primary"
                          >
                            <ChevronDown className="w-3.5 h-3.5 group-hover/more:translate-y-0.5 transition-transform" />
                            <span>Xem thêm</span>
                            <span className="px-2 py-0.5 rounded-full bg-zinc-100 dark:bg-white/5 group-hover/more:bg-primary/10 text-[9px] tracking-wider">
                              {remaining}
                            </span>
                          </button>
                        )}
...
const COLOR_TOKENS: Record<string, ColorTokens> = {
  blue: {
    bg: 'bg-gradient-to-br from-blue-50/40 to-sky-50/20 dark:from-blue-500/[0.035] dark:to-sky-500/[0.015]',
    border: 'border-blue-200/40 dark:border-blue-500/10 hover:border-blue-300/50 dark:hover:border-blue-400/20',
    stripe: 'bg-gradient-to-b from-blue-300 to-sky-400 dark:from-blue-400/70 dark:to-sky-400/70',
    shadow: 'hover:shadow-blue-200/20 dark:hover:shadow-blue-500/5',
    title: 'text-zinc-700 dark:text-zinc-300',
    accent: 'text-blue-500/80 dark:text-blue-300/80',
    glow: 'bg-blue-300',
  },
  green: {
    bg: 'bg-gradient-to-br from-emerald-50/40 to-teal-50/20 dark:from-emerald-500/[0.035] dark:to-teal-500/[0.015]',
    border: 'border-emerald-200/40 dark:border-emerald-500/10 hover:border-emerald-300/50 dark:hover:border-emerald-400/20',
    stripe: 'bg-gradient-to-b from-emerald-300 to-teal-400 dark:from-emerald-400/70 dark:to-teal-400/70',
    shadow: 'hover:shadow-emerald-200/20 dark:hover:shadow-emerald-500/5',
    title: 'text-zinc-700 dark:text-zinc-300',
    accent: 'text-emerald-500/80 dark:text-emerald-300/80',
    glow: 'bg-emerald-300',
  },
  pink: {
    bg: 'bg-gradient-to-br from-rose-50/40 to-pink-50/20 dark:from-rose-500/[0.035] dark:to-pink-500/[0.015]',
    border: 'border-rose-200/40 dark:border-rose-500/10 hover:border-rose-300/50 dark:hover:border-rose-400/20',
    stripe: 'bg-gradient-to-b from-rose-300 to-pink-400 dark:from-rose-400/70 dark:to-pink-400/70',
    shadow: 'hover:shadow-rose-200/20 dark:hover:shadow-rose-500/5',
    title: 'text-zinc-700 dark:text-zinc-300',
    accent: 'text-rose-500/80 dark:text-rose-300/80',
    glow: 'bg-rose-300',
  },
  yellow: {
    bg: 'bg-gradient-to-br from-amber-50/40 to-orange-50/20 dark:from-amber-500/[0.035] dark:to-orange-500/[0.015]',
    border: 'border-amber-200/40 dark:border-amber-500/10 hover:border-amber-300/50 dark:hover:border-amber-400/20',
    stripe: 'bg-gradient-to-b from-amber-300 to-orange-300 dark:from-amber-400/70 dark:to-orange-400/70',
    shadow: 'hover:shadow-amber-200/20 dark:hover:shadow-amber-500/5',
    title: 'text-zinc-700 dark:text-zinc-300',
    accent: 'text-amber-600/80 dark:text-amber-300/80',
    glow: 'bg-amber-300',
  },
};
...
function NoteCard({
  ...
  return (
    ...
    >
      {/* Left color stripe with gradient */}
      <div className={`absolute left-0 top-0 bottom-0 w-[2.5px] opacity-80 ${tokens.stripe}`} />
      ...
      <div className="relative pl-4 pr-3 py-3.5 space-y-2.5">
