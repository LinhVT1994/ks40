...
export default function EyeTracker() {
  ...
  const renderEye = (ref: React.RefObject<HTMLDivElement | null>) => {
    ...
    if (!isMobile) {
      ...
      dilation = Math.max(0.8, 1.2 - Math.min(distRaw / 500, 0.4));
    }

    return (
      <div
        ref={ref}
        className="w-5 h-5 sm:w-6 sm:h-6 bg-white dark:bg-slate-900 rounded-full border border-zinc-200 dark:border-white/20 relative flex items-center justify-center overflow-hidden shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)] dark:shadow-[inset_0_2px_4px_rgba(0,0,0,0.4)]"
        style={{
      ...
      >
        {!isBlinking && (
          <div
            className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-gradient-to-br from-blue-500 via-primary to-blue-700 rounded-full absolute shadow-sm flex items-center justify-center"
            style={{
